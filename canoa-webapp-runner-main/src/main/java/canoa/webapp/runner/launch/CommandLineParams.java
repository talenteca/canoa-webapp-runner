package canoa.webapp.runner.launch;
...
public class CommandLineParams {
  ...
  @Parameter(
      names = "--session-store",
      description = "Session store to use (default is none)")
  ...
  @Parameter(
      ...
      description = "Operation timeout for the session store. (default is 5000ms)")
  public Integer sessionStoreOperationTimout = 5000;

  @Parameter(
      names = "--session-store-locking-mode",
      description =
          "Session locking mode for use with session store. (default is all)")
  public String sessionStoreLockingMode = "all";

  @Parameter(
      names = "--session-store-ignore-pattern",
      description =
          "Request pattern to not track sessions for. Valid only with session store."
              + " (default is '.*\\.(png|gif|jpg|css|js)$')")
  public String sessionStoreIgnorePattern = ".*\\.(png|gif|jpg|css|js)$";

  @Parameter(
      names = "--session-store-ssl-endpoint-identification",
      description =
          "Enables or disables SSL endpoint identification for the session store. (default is"
              + " true)",
      arity = 1)
  ...
  @Parameter(
      ...
      description =
          "Location of the tomcat-users.xml file. (relative to the location of the canoa-webapp-runner"
              + " jar file)")
